...
class ReminderSettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSetTime.setOnClickListener {
            ...
            TimePickerDialog(this, { _, selectedHour, selectedMinute ->
                ...
                // Add to history
                saveReminderToHistory("Daily reminder set for: %02d:%02d".format(selectedHour, selectedMinute))
                loadReminderHistory()
    ...
    private fun saveReminderToHistory(message: String) {
        val prefs = getSharedPreferences("ReminderHistory", Context.MODE_PRIVATE)
        val history = prefs.getString("history", "") ?: ""
        val updatedHistory = "$message\n$history"
        prefs.edit().putString("history", updatedHistory).apply()
    }

    private fun loadReminderHistory() {
        val prefs = getSharedPreferences("ReminderHistory", Context.MODE_PRIVATE)
        val history = prefs.getString("history", "No completed reminders yet.")
